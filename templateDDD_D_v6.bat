...
REM ============================================================
REM === PROYECTO DE TESTS DE API (CONTROLLERS) ===
REM ============================================================
...
echo.
echo ============================================================
echo === CREANDO PROYECTO DE TESTS DE API ===
echo ============================================================

cd "%projectDirectory%\tests"
dotnet new xunit -o "%projectName%.API.Tests"
if exist "%projectName%.API.Tests\UnitTest1.cs" del /f /q "%projectName%.API.Tests\UnitTest1.cs"
dotnet sln "%projectDirectory%\src\%projectName%.sln" add "%projectName%.API.Tests"
dotnet add "%projectName%.API.Tests" reference "%projectDirectory%\src\%projectName%.API\%projectName%.API.csproj"
dotnet add "%projectName%.API.Tests" reference "%projectDirectory%\src\%projectName%.Application\%projectName%.Application.csproj"
dotnet add "%projectName%.API.Tests" reference "%projectDirectory%\src\%projectName%.Domain\%projectName%.Domain.csproj"

cd "%projectName%.API.Tests"
dotnet add package Moq
dotnet add package FluentAssertions
...
REM ============================================================
REM === CUSTOMER + ORDER COMMANDS/QUERIES (stub para controllers) ===
REM ============================================================

mkdir "%projectName%.Application\Commands\Customers" 2>nul
(
echo using MediatR;
echo using %projectName%.Domain.Common;
echo.
echo namespace %projectName%.Application.Commands.Customers;
echo.
echo public record CreateCustomerCommand : IRequest^<CustomerId^>
echo {
...
echo }
echo.
echo public record UpdateCustomerEmailCommand : IRequest^<bool^>
echo {
echo     public int    CustomerId { get; init; }
echo     public string NewEmail   { get; init; } = string.Empty;
echo }
echo.
echo public record DeactivateCustomerCommand : IRequest^<bool^>
echo {
echo     public int CustomerId { get; init; }
echo }
) > "%projectName%.Application\Commands\Customers\CustomerCommands.cs"

mkdir "%projectName%.Application\Queries\Customers" 2>nul
(
echo using MediatR;
echo using %projectName%.Application.DTOs;
echo.
echo namespace %projectName%.Application.Queries.Customers;
echo.
echo public record GetCustomersQuery : IRequest^<IEnumerable^<CustomerDto^>^>;
echo public record GetCustomerByIdQuery^(int Id^) : IRequest^<CustomerDto?^>;
) > "%projectName%.Application\Queries\Customers\CustomerQueries.cs"

mkdir "%projectName%.Application\Commands\Orders" 2>nul
(
echo using MediatR;
echo using %projectName%.Domain.Common;
echo.
echo namespace %projectName%.Application.Commands.Orders;
echo.
echo public record CreateOrderCommand : IRequest^<OrderId^>
echo {
echo     public int CustomerId { get; init; }
echo }
echo.
echo public record AddOrderItemCommand : IRequest^<bool^>
echo {
echo     public int     OrderId   { get; init; }
echo     public int     ProductId { get; init; }
...
echo }
echo.
echo public record ConfirmOrderCommand : IRequest^<bool^>
echo {
echo     public int OrderId { get; init; }
echo }
echo.
echo public record CancelOrderCommand : IRequest^<bool^>
echo {
echo     public int OrderId { get; init; }
echo }
) > "%projectName%.Application\Commands\Orders\OrderCommands.cs"

mkdir "%projectName%.Application\Queries\Orders" 2>nul
(
echo using MediatR;
echo.
echo namespace %projectName%.Application.Queries.Orders;
echo.
echo public record GetOrdersQuery : IRequest^<IEnumerable^<object^>^>;
echo public record GetOrderByIdQuery^(int Id^) : IRequest^<object?^>;
echo public record GetOrdersByCustomerQuery^(int CustomerId^) : IRequest^<IEnumerable^<object^>^>;
) > "%projectName%.Application\Queries\Orders\OrderQueries.cs"

REM ============================================================
REM === CustomersController ===
REM ============================================================

(
echo using MediatR;
echo using Microsoft.AspNetCore.Mvc;
echo using %projectName%.Application.Commands.Customers;
echo using %projectName%.Application.Queries.Customers;
echo.
echo namespace %projectName%.API.Controllers;
echo.
echo [ApiController]
...
echo     private readonly IMediator _mediator;
echo.
echo     public CustomersController^(IMediator mediator^) =^> _mediator = mediator;
echo.
echo     [HttpGet]
echo     public async Task^<IActionResult^> GetAll^(CancellationToken ct^)
echo         =^> Ok^(await _mediator.Send^(new GetCustomersQuery^(^), ct^)^);
echo.
echo     [HttpGet^("{id}"^)]
echo     public async Task^<IActionResult^> GetById^(int id, CancellationToken ct^)
echo     {
echo         var result = await _mediator.Send^(new GetCustomerByIdQuery^(id^), ct^);
echo         return result == null ? NotFound^(^) : Ok^(result^);
echo     }
echo.
echo     [HttpPost]
echo     public async Task^<IActionResult^> Create^([FromBody] CreateCustomerCommand command, CancellationToken ct^)
echo     {
echo         var customerId = await _mediator.Send^(command, ct^);
echo         return CreatedAtAction^(nameof^(GetById^), new { id = customerId.Value }, customerId^);
echo     }
echo.
echo     [HttpPut^("{id}/email"^)]
echo     public async Task^<IActionResult^> UpdateEmail^(int id, [FromBody] UpdateCustomerEmailCommand command, CancellationToken ct^)
echo     {
echo         if ^(id != command.CustomerId^) return BadRequest^(^);
echo         var result = await _mediator.Send^(command, ct^);
echo         return result ? NoContent^(^) : NotFound^(^);
echo     }
echo.
...
echo     public async Task^<IActionResult^> Deactivate^(int id, CancellationToken ct^)
echo     {
echo         var result = await _mediator.Send^(new DeactivateCustomerCommand { CustomerId = id }, ct^);
echo         return result ? NoContent^(^) : NotFound^(^);
echo     }
echo }
) > "%projectName%.API\Controllers\CustomersController.cs"

REM ============================================================
REM === OrdersController ===
REM ============================================================

(
echo using MediatR;
echo using Microsoft.AspNetCore.Mvc;
echo using %projectName%.Application.Commands.Orders;
echo using %projectName%.Application.Queries.Orders;
echo.
echo namespace %projectName%.API.Controllers;
echo.
echo [ApiController]
...
echo     private readonly IMediator _mediator;
echo.
echo     public OrdersController^(IMediator mediator^) =^> _mediator = mediator;
echo.
echo     [HttpGet]
echo     public async Task^<IActionResult^> GetAll^(CancellationToken ct^)
echo         =^> Ok^(await _mediator.Send^(new GetOrdersQuery^(^), ct^)^);
echo.
echo     [HttpGet^("{id}"^)]
echo     public async Task^<IActionResult^> GetById^(int id, CancellationToken ct^)
echo     {
echo         var result = await _mediator.Send^(new GetOrderByIdQuery^(id^), ct^);
echo         return result == null ? NotFound^(^) : Ok^(result^);
echo     }
echo.
echo     [HttpGet^("customer/{customerId}"^)]
echo     public async Task^<IActionResult^> GetByCustomer^(int customerId, CancellationToken ct^)
echo         =^> Ok^(await _mediator.Send^(new GetOrdersByCustomerQuery^(customerId^), ct^)^);
echo.
echo     [HttpPost]
echo     public async Task^<IActionResult^> Create^([FromBody] CreateOrderCommand command, CancellationToken ct^)
echo     {
echo         var orderId = await _mediator.Send^(command, ct^);
echo         return CreatedAtAction^(nameof^(GetById^), new { id = orderId.Value }, orderId^);
echo     }
echo.
echo     [HttpPost^("{id}/items"^)]
echo     public async Task^<IActionResult^> AddItem^(int id, [FromBody] AddOrderItemCommand command, CancellationToken ct^)
echo     {
echo         if ^(id != command.OrderId^) return BadRequest^(^);
echo         var result = await _mediator.Send^(command, ct^);
echo         return result ? NoContent^(^) : NotFound^(^);
echo     }
echo.
...
echo     {
echo         var result = await _mediator.Send^(new ConfirmOrderCommand { OrderId = id }, ct^);
echo         return result ? NoContent^(^) : NotFound^(^);
echo     }
echo.
...
echo     {
echo         var result = await _mediator.Send^(new CancelOrderCommand { OrderId = id }, ct^);
echo         return result ? NoContent^(^) : NotFound^(^);
echo     }
echo }
) > "%projectName%.API\Controllers\OrdersController.cs"

REM ============================================================
REM === TESTS: ProductsController ===
REM ============================================================
...
echo using Moq;
echo using FluentAssertions;
echo using %projectName%.API.Controllers;
echo using %projectName%.Application.Commands.Products;
echo using %projectName%.Application.Queries.Products;
echo using %projectName%.Domain.Common;
echo using Xunit;
echo.
echo namespace %projectName%.API.Tests;
echo.
echo public class ProductsControllerTests
echo {
echo     private readonly Mock^<IMediator^> _mediatorMock;
echo     private readonly ProductsController _controller;
echo.
echo     public ProductsControllerTests^(^)
echo     {
echo         _mediatorMock = new Mock^<IMediator^>^(^);
echo         _controller = new ProductsController^(_mediatorMock.Object^);
echo     }
echo.
echo     // ── GET /api/products ──────────────────────────────────────
echo     [Fact]
echo     public async Task GetAll_ShouldReturn200Ok_WithProductList^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetProductsQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^>^(^)^);
echo.
echo         var result = await _controller.GetAll^(CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<GetProductsQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── GET /api/products/{id} found ────────────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn200Ok_WhenProductFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetProductByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new { Id = 1, Name = "Laptop" }^);
echo.
echo         var result = await _controller.GetById^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo         ^(^(OkObjectResult^)result^).StatusCode.Should^(^).Be^(200^);
echo     }
echo.
echo     // ── GET /api/products/{id} not found ────────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn404_WhenProductNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetProductByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(^(object?^)null^);
echo.
echo         var result = await _controller.GetById^(999, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── GET /api/products/active ────────────────────────────────
echo     [Fact]
echo     public async Task GetActive_ShouldReturn200Ok^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetActiveProductsQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^>^(^)^);
echo.
echo         var result = await _controller.GetActive^(CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo     }
echo.
...
echo     [Fact]
echo     public async Task GetInStock_ShouldReturn200Ok^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetProductsInStockQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^>^(^)^);
echo.
echo         var result = await _controller.GetInStock^(CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo     }
echo.
echo     // ── GET /api/products/price-range ───────────────────────────
echo     [Fact]
echo     public async Task GetByPriceRange_ShouldReturn200Ok_AndPassCorrectQuery^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetProductsByPriceRangeQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^>^(^)^);
echo.
echo         var result = await _controller.GetByPriceRange^(10, 500, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<GetProductsByPriceRangeQuery^>^(q =^> q.MinPrice == 10 ^&^& q.MaxPrice == 500^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── POST /api/products ──────────────────────────────────────
echo     [Fact]
echo     public async Task Create_ShouldReturn201Created_WithLocation^(^)
echo     {
echo         var newId = ProductId.From^(42^);
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<CreateProductCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(newId^);
echo.
echo         var command = new CreateProductCommand { Name = "Laptop", Price = 999, Currency = "USD", Description = "Desc" };
echo.
echo         var result = await _controller.Create^(command, CancellationToken.None^);
echo.
echo         var created = result.Should^(^).BeOfType^<CreatedAtActionResult^>^(^).Subject;
echo         created.StatusCode.Should^(^).Be^(201^);
echo         created.RouteValues!["id"].Should^(^).Be^(42^);
echo     }
echo.
echo     // ── PUT /api/products/{id}/price - success ──────────────────
echo     [Fact]
echo     public async Task UpdatePrice_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<UpdateProductPriceCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var command = new UpdateProductPriceCommand { ProductId = 1, NewPrice = 1500, Currency = "USD" };
echo.
echo         var result = await _controller.UpdatePrice^(1, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo     }
echo.
echo     // ── PUT /api/products/{id}/price - not found ────────────────
echo     [Fact]
echo     public async Task UpdatePrice_ShouldReturn404_WhenProductNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<UpdateProductPriceCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var command = new UpdateProductPriceCommand { ProductId = 99, NewPrice = 100, Currency = "USD" };
echo.
echo         var result = await _controller.UpdatePrice^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── PUT /api/products/{id}/price - id mismatch ──────────────
echo     [Fact]
echo     public async Task UpdatePrice_ShouldReturn400_WhenIdMismatch^(^)
echo     {
echo         var command = new UpdateProductPriceCommand { ProductId = 5, NewPrice = 100, Currency = "USD" };
echo.
echo         var result = await _controller.UpdatePrice^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<BadRequestResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<UpdateProductPriceCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Never^);
echo     }
echo.
echo     // ── POST /api/products/{id}/stock/add - success ─────────────
echo     [Fact]
echo     public async Task AddStock_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<AddStockCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.AddStock^(1, new AddStockCommand { ProductId = 1, Quantity = 10 }, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo     }
echo.
echo     // ── POST /api/products/{id}/stock/add - not found ───────────
echo     [Fact]
echo     public async Task AddStock_ShouldReturn404_WhenProductNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<AddStockCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var result = await _controller.AddStock^(99, new AddStockCommand { ProductId = 99, Quantity = 5 }, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── POST /api/products/{id}/stock/add - id mismatch ─────────
echo     [Fact]
echo     public async Task AddStock_ShouldReturn400_WhenIdMismatch^(^)
echo     {
echo         var result = await _controller.AddStock^(99, new AddStockCommand { ProductId = 5, Quantity = 3 }, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<BadRequestResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<AddStockCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Never^);
echo     }
echo.
echo     // ── POST /api/products/{id}/stock/remove - success ──────────
echo     [Fact]
echo     public async Task RemoveStock_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<RemoveStockCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.RemoveStock^(1, new RemoveStockCommand { ProductId = 1, Quantity = 3 }, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo     }
echo.
echo     // ── POST /api/products/{id}/stock/remove - id mismatch ──────
echo     [Fact]
echo     public async Task RemoveStock_ShouldReturn400_WhenIdMismatch^(^)
echo     {
echo         var result = await _controller.RemoveStock^(99, new RemoveStockCommand { ProductId = 5, Quantity = 3 }, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<BadRequestResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<RemoveStockCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Never^);
echo     }
echo.
echo     // ── POST /api/products/{id}/deactivate - success ────────────
echo     [Fact]
echo     public async Task Deactivate_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<DeactivateProductCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.Deactivate^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<DeactivateProductCommand^>^(c =^> c.ProductId == 1^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── POST /api/products/{id}/deactivate - not found ──────────
echo     [Fact]
echo     public async Task Deactivate_ShouldReturn404_WhenProductNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<DeactivateProductCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var result = await _controller.Deactivate^(999, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo }
) > "%projectName%.API.Tests\ProductsControllerTests.cs"

REM ============================================================
REM === TESTS: CustomersController ===
REM ============================================================
...
echo namespace %projectName%.API.Tests;
echo.
echo public class CustomersControllerTests
echo {
echo     private readonly Mock^<IMediator^> _mediatorMock;
echo     private readonly CustomersController _controller;
echo.
echo     public CustomersControllerTests^(^)
echo     {
echo         _mediatorMock = new Mock^<IMediator^>^(^);
echo         _controller = new CustomersController^(_mediatorMock.Object^);
echo     }
echo.
echo     // ── GET /api/customers ──────────────────────────────────────
echo     [Fact]
echo     public async Task GetAll_ShouldReturn200Ok_WithCustomerList^(^)
echo     {
echo         var customers = new List^<CustomerDto^>
echo         {
echo             new CustomerDto { Id = 1, FirstName = "Alice", LastName = "Smith", Email = "alice@x.com" },
echo             new CustomerDto { Id = 2, FirstName = "Bob",   LastName = "Jones", Email = "bob@x.com"   }
echo         };
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetCustomersQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(customers^);
echo.
echo         var result = await _controller.GetAll^(CancellationToken.None^);
echo.
echo         var ok = result.Should^(^).BeOfType^<OkObjectResult^>^(^).Subject;
echo         ok.StatusCode.Should^(^).Be^(200^);
echo         ^(^(IEnumerable^<CustomerDto^>^)ok.Value!^).Should^(^).HaveCount^(2^);
echo     }
echo.
echo     // ── GET /api/customers/{id} found ───────────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn200Ok_WhenCustomerFound^(^)
echo     {
echo         var dto = new CustomerDto { Id = 1, FirstName = "Alice", LastName = "Smith", Email = "alice@x.com" };
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetCustomerByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(dto^);
echo.
echo         var result = await _controller.GetById^(1, CancellationToken.None^);
echo.
echo         var ok = result.Should^(^).BeOfType^<OkObjectResult^>^(^).Subject;
echo         ^(^(CustomerDto^)ok.Value!^).FirstName.Should^(^).Be^("Alice"^);
echo     }
echo.
echo     // ── GET /api/customers/{id} not found ───────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn404_WhenCustomerNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetCustomerByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(^(CustomerDto?^)null^);
echo.
echo         var result = await _controller.GetById^(999, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
...
echo     public async Task Create_ShouldReturn201Created_WithLocation^(^)
echo     {
echo         var newId = CustomerId.From^(7^);
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<CreateCustomerCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(newId^);
echo.
echo         var command = new CreateCustomerCommand { FirstName = "Alice", LastName = "Smith", Email = "alice@example.com" };
echo.
echo         var result = await _controller.Create^(command, CancellationToken.None^);
echo.
echo         var created = result.Should^(^).BeOfType^<CreatedAtActionResult^>^(^).Subject;
echo         created.StatusCode.Should^(^).Be^(201^);
echo         created.RouteValues!["id"].Should^(^).Be^(7^);
echo     }
echo.
echo     // ── PUT /api/customers/{id}/email - success ─────────────────
echo     [Fact]
echo     public async Task UpdateEmail_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<UpdateCustomerEmailCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var command = new UpdateCustomerEmailCommand { CustomerId = 1, NewEmail = "new@example.com" };
echo.
echo         var result = await _controller.UpdateEmail^(1, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo     }
echo.
echo     // ── PUT /api/customers/{id}/email - not found ───────────────
echo     [Fact]
echo     public async Task UpdateEmail_ShouldReturn404_WhenCustomerNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<UpdateCustomerEmailCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var command = new UpdateCustomerEmailCommand { CustomerId = 99, NewEmail = "x@x.com" };
echo.
echo         var result = await _controller.UpdateEmail^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── PUT /api/customers/{id}/email - id mismatch ─────────────
echo     [Fact]
echo     public async Task UpdateEmail_ShouldReturn400_WhenIdMismatch^(^)
echo     {
echo         var command = new UpdateCustomerEmailCommand { CustomerId = 5, NewEmail = "x@x.com" };
echo.
echo         var result = await _controller.UpdateEmail^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<BadRequestResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<UpdateCustomerEmailCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Never^);
echo     }
echo.
echo     // ── POST /api/customers/{id}/deactivate - success ───────────
echo     [Fact]
echo     public async Task Deactivate_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<DeactivateCustomerCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.Deactivate^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<DeactivateCustomerCommand^>^(c =^> c.CustomerId == 1^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── POST /api/customers/{id}/deactivate - not found ─────────
echo     [Fact]
echo     public async Task Deactivate_ShouldReturn404_WhenCustomerNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<DeactivateCustomerCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
...
echo     }
echo }
) > "%projectName%.API.Tests\CustomersControllerTests.cs"

REM ============================================================
REM === TESTS: OrdersController ===
REM ============================================================
...
echo namespace %projectName%.API.Tests;
echo.
echo public class OrdersControllerTests
echo {
echo     private readonly Mock^<IMediator^> _mediatorMock;
echo     private readonly OrdersController _controller;
echo.
echo     public OrdersControllerTests^(^)
echo     {
echo         _mediatorMock = new Mock^<IMediator^>^(^);
echo         _controller = new OrdersController^(_mediatorMock.Object^);
echo     }
echo.
echo     // ── GET /api/orders ─────────────────────────────────────────
echo     [Fact]
echo     public async Task GetAll_ShouldReturn200Ok^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetOrdersQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^>^(^)^);
echo.
echo         var result = await _controller.GetAll^(CancellationToken.None^);
echo.
...
echo     }
echo.
echo     // ── GET /api/orders/{id} found ──────────────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn200Ok_WhenOrderFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetOrderByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new { Id = 1, Status = "Pending" }^);
echo.
echo         var result = await _controller.GetById^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo     }
echo.
echo     // ── GET /api/orders/{id} not found ──────────────────────────
echo     [Fact]
echo     public async Task GetById_ShouldReturn404_WhenOrderNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetOrderByIdQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(^(object?^)null^);
...
echo     }
echo.
echo     // ── GET /api/orders/customer/{customerId} ───────────────────
echo     [Fact]
echo     public async Task GetByCustomer_ShouldReturn200Ok_AndPassCorrectQuery^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<GetOrdersByCustomerQuery^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(new List^<object^> { new { Id = 1 }, new { Id = 2 } }^);
echo.
echo         var result = await _controller.GetByCustomer^(3, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<OkObjectResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<GetOrdersByCustomerQuery^>^(q =^> q.CustomerId == 3^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
...
echo     public async Task Create_ShouldReturn201Created_WithLocation^(^)
echo     {
echo         var newId = OrderId.From^(10^);
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<CreateOrderCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(newId^);
echo.
echo         var result = await _controller.Create^(new CreateOrderCommand { CustomerId = 5 }, CancellationToken.None^);
echo.
echo         var created = result.Should^(^).BeOfType^<CreatedAtActionResult^>^(^).Subject;
echo         created.StatusCode.Should^(^).Be^(201^);
echo         created.RouteValues!["id"].Should^(^).Be^(10^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/items - success ───────────────────
echo     [Fact]
echo     public async Task AddItem_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<AddOrderItemCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var command = new AddOrderItemCommand { OrderId = 1, ProductId = 10, UnitPrice = 100, Quantity = 2 };
echo.
echo         var result = await _controller.AddItem^(1, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/items - id mismatch ───────────────
echo     [Fact]
echo     public async Task AddItem_ShouldReturn400_WhenIdMismatch^(^)
echo     {
echo         var command = new AddOrderItemCommand { OrderId = 5, ProductId = 10, UnitPrice = 100, Quantity = 2 };
echo.
echo         var result = await _controller.AddItem^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<BadRequestResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(It.IsAny^<AddOrderItemCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^), Times.Never^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/items - not found ─────────────────
echo     [Fact]
echo     public async Task AddItem_ShouldReturn404_WhenOrderNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<AddOrderItemCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var command = new AddOrderItemCommand { OrderId = 99, ProductId = 1, UnitPrice = 50, Quantity = 1 };
echo.
echo         var result = await _controller.AddItem^(99, command, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/confirm - success ─────────────────
echo     [Fact]
echo     public async Task Confirm_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<ConfirmOrderCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.Confirm^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<ConfirmOrderCommand^>^(c =^> c.OrderId == 1^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/confirm - not found ───────────────
echo     [Fact]
echo     public async Task Confirm_ShouldReturn404_WhenOrderNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<ConfirmOrderCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var result = await _controller.Confirm^(999, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/cancel - success ──────────────────
echo     [Fact]
echo     public async Task Cancel_ShouldReturn204_WhenSuccess^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<CancelOrderCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(true^);
echo.
echo         var result = await _controller.Cancel^(1, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NoContentResult^>^(^);
echo         _mediatorMock.Verify^(m =^> m.Send^(
echo             It.Is^<CancelOrderCommand^>^(c =^> c.OrderId == 1^),
echo             It.IsAny^<CancellationToken^>^(^)^), Times.Once^);
echo     }
echo.
echo     // ── POST /api/orders/{id}/cancel - not found ────────────────
echo     [Fact]
echo     public async Task Cancel_ShouldReturn404_WhenOrderNotFound^(^)
echo     {
echo         _mediatorMock
echo             .Setup^(m =^> m.Send^(It.IsAny^<CancelOrderCommand^>^(^), It.IsAny^<CancellationToken^>^(^)^)^)
echo             .ReturnsAsync^(false^);
echo.
echo         var result = await _controller.Cancel^(999, CancellationToken.None^);
echo.
echo         result.Should^(^).BeOfType^<NotFoundResult^>^(^);
echo     }
echo }
) > "%projectName%.API.Tests\OrdersControllerTests.cs"
