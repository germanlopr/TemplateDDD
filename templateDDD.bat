...
REM Query Handlers segÃºn modo
echo Query Handlers segÃºn modo
if "%cqrsMode%" == "Light" (
    (
...
echo }
        ...
        REM Query Handlers para CQRS Real con Dapper
        (
echo using MediatR;
echo using %projectName%.Application.Common.Interfaces;
echo using %projectName%.Application.ReadModels;
...
echo     public async Task^<IEnumerable^<ProductReadModel^>^> Handle^(GetProductsByPriceRangeQuery request, CancellationToken ct^)
echo     {
echo         const string sql = @"SELECT * FROM ProductsReadModel 
echo             WHERE Price ^>= @MinPrice AND Price ^<= @MaxPrice 
echo             ORDER BY Price";
...
echo     {
echo         const string sql = @"INSERT INTO ProductsReadModel ^(Id, Name, Price, Currency, Description, IsActive, Stock, CreatedAt^)
echo             VALUES ^(@Id, @Name, @Price, @Currency, @Description, @IsActive, @Stock, @CreatedAt^)";
...
echo     {
echo         const string sql = @"UPDATE ProductsReadModel SET Name=@Name, Price=@Price, Currency=@Currency,
echo             Description=@Description, IsActive=@IsActive, Stock=@Stock, UpdatedAt=@UpdatedAt WHERE Id=@Id";
echo         using var conn = new SqlConnection^(_connectionString^);
echo         await conn.ExecuteAsync^(sql, model^);
echo     }
echo.
...
echo     {
echo         const string sql = @"INSERT INTO CustomersReadModel ^(Id, FirstName, LastName, FullName, Email, IsActive, CreatedAt^)
echo             VALUES ^(@Id, @FirstName, @LastName, @FullName, @Email, @IsActive, @CreatedAt^)";
echo         using var conn = new SqlConnection^(_connectionString^);
echo         await conn.ExecuteAsync^(sql, model^);
echo     }
echo.
...
echo     {
echo         const string sql = @"UPDATE CustomersReadModel SET FirstName=@FirstName, LastName=@LastName,
echo             FullName=@FullName, Email=@Email, IsActive=@IsActive WHERE Id=@Id";
echo         using var conn = new SqlConnection^(_connectionString^);
echo         await conn.ExecuteAsync^(sql, model^);
echo     }
echo.
...
echo     {
echo         const string sql = @"INSERT INTO OrdersReadModel ^(Id, CustomerId, CustomerName, OrderDate, Status, Total, Currency, ItemCount^)
echo             VALUES ^(@Id, @CustomerId, @CustomerName, @OrderDate, @Status, @Total, @Currency, @ItemCount^)";
echo         using var conn = new SqlConnection^(_connectionString^);
echo         await conn.ExecuteAsync^(sql, model^);
echo     }
echo.
...
echo     {
echo         const string sql = @"UPDATE OrdersReadModel SET CustomerId=@CustomerId, CustomerName=@CustomerName,
echo             OrderDate=@OrderDate, Status=@Status, Total=@Total, Currency=@Currency, ItemCount=@ItemCount WHERE Id=@Id";
echo         using var conn = new SqlConnection^(_connectionString^);
echo         await conn.ExecuteAsync^(sql, model^);
echo     }
echo.
...
REM ============================================================
REM === GENERANDO CÓDIGO DE TEST PARA APPLICATION ===
REM ============================================================
...
REM ============================================================
REM === GENERANDO CÓDIGO DE TEST PARA INFRASTRUCTURE ===
REM ============================================================
